...
export default function TransactionForm({
  ...
  const color     = esIngreso ? 'var(--success-green)' : 'var(--red-alert)'
  const colorLight = esIngreso ? 'var(--success-green-light)' : 'var(--red-light)'
    ...
    <div onClick={onClose} style={overlay}>
      <div onClick={e => e.stopPropagation()} style={modal}>
        ...
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:20 }}>
          <div>
            <div style={{ fontFamily:'Nunito', fontWeight:900, fontSize:18, color:'var(--text)' }}>
        ...
        {!tipoInicial && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginBottom:18 }}>
            {[
              { v:'ingreso', e:'↑', label:'Ingreso',  c:'var(--success-green)' },
              { v:'gasto',   e:'↓', label:'Gasto',    c:'var(--red-alert)' },
            ].map(t => (
          ...
          <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6 }}>
            {categorias.map(c => (
              <div key={c.valor} onClick={() => setCategoria(c.valor)} style={{
                ...
                <div style={{
                  fontSize:10, 
                  fontWeight: categoria===c.valor ? 700 : 400,
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <button onClick={guardar} disabled={loading} style={{
            ...
            border:'none', 
            borderRadius:10,
            background: loading ? 'var(--gray-400)' : color,
            color:'white', 
            fontFamily:'Poppins', 
            fontWeight:700,
            fontSize:13, 
